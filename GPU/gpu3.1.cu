...
__global__ void addBias(__half *Z, int bias = 1, int m, int k) 
{
    ...
    {
        if (bias == 1)
            Z[row * k + col] += const_bias1[col];
        else if (bias == 2)
            Z[row * k + col] += const_bias2[col];
        else if (bias == 3)
            Z[row * k + col] += const_bias3[col];
    }
...
void initANN(ANN *nn, __half *X_train, __half *Y_train, __half *X_valid, __half *Y_valid, __half *X_test, __half *Y_test, int BATCH_SIZE)
{
    ...
    b2 = (__half*)malloc(128 * sizeof(__half));
    b3 = (__half*)malloc(10  * sizeof(__half));
    ...
    CHECK(cudaMemcpyToSymbol(const_bias1, b1, 128 * sizeof(__half)));
    CHECK(cudaMemcpyToSymbol(const_bias2, b2, 128 * sizeof(__half)));
    CHECK(cudaMemcpyToSymbol(const_bias3, b3, 10 * sizeof(__half)));
